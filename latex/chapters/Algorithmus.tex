...
%\begin{theorem}[Cauchysche Integralsatz]
%Es seien $D \in \mathbb{C}$ offen und $f: D \rightarrow \mathbb{C}$ eine holomorphe Funktion.
% Ferner seien $Q = [a,b] \times [c, d] \in \mathbb{R}^2$ ein Quader,
% $\Psi(\partial Q)$ das Bild der Randkurve von $Q$ unter $\Psi$. Dann gilt $\int_{\gamma} f(z) \,dz = 0$.    
%\end{theorem}


%In der Praxis wird der Cauchysche Integralsatz meistens in einer äquivalenten Umformulierung verwendet, die wir nun genauer behandeln wollen. Anschaulich besagt sie, dass Wegintegrale 
%$\int_{}^{\gamma} f(z)dz\,dz $
%ihren Wert nicht ändern, wenn man den Weg $\gamma$ innerhalb des Bereiches deformiert, in dem $f$ holomorph ist. 

%Um eine solche Aussage mathematisch exakt formulieren zu können, müssen wir natürlich zunächst einmal definieren, was wir unter einer "Deformation eines Weges" genau verstehen
%wollen. Der korrekte mathematische Begriff hierfür ist die sogenannte Homotopie. Dieses Konzept
%stammt eigentlich aus der Topologie und wird normalerweise für lediglich stetige Wege formuliert
...
\begin{table}
    ...
    \begin{tabular}{|c|c|c|c|}
        \hline
        \multicolumn{2}{|c|}{Fall} & $c_s$ & $c_r$ \\
        \hline
        \multicolumn{2}{|c|}{$|q|=\infty$} & ${c, \overline{c}}$ & $Re(c)$ \\
        \hline
        \multicolumn{2}{|c|}{$|q|=0$} & $Re(c)$  & ${c, \overline{c}}$\\
        \hline
        \multicolumn{2}{|c|}{$|q|=\sqrt{c_0}$} & $|c_s| = \infty$ & $|c_r| = \infty$ \\
        \hline
        \multirow{2}*{$|q|<\sqrt{c_0}$} & $q < 0$ & $Re(c) + K_{c_s}$ & $Re(c) + K_{c_r}$ \\
        \cline{2-3} & $q > 0$ & $Re(c) - K_{c_s}$ & $Re(c) - K_{c_r}$ \\
        \hline
        \multirow{2}*{$|q|>\sqrt{c_0}$} & $q < 0$ & $Re(c) + K_{c_s}$ & $Re(c) + K_{c_r}$ \\
        \cline{2-3} & $q > 0$ & $Re(c) - K_{c_s}$ & $Re(c) - K_{c_r}$ \\
        \hline
...
%Der Cauchysche Integralsatz ist ein Satz der Funktionentheorie.
...
%Diese wird in dem steepest-descent Verfahren in Kombination mit dem Cauchysche Integralsatz genutzt um das Integral "LINK AUF FORMEL" zu berechnen.
